...
CIvfUserSettingsPtr CIvfFibre::m_userSettings = NULL;
...
void CIvfFibre::createGeometry()
{
	...
	if (m_userSettings!=NULL)
		g_radius = m_userSettings->getFibreRadius();
	else
		g_radius = 0.5;
	
	if (m_userSettings!=NULL)
		g_nSides = m_userSettings->getExtrusionSides();
	else
		g_nSides = 6;
	
	if (m_userSettings!=NULL)
		g_scale = m_userSettings->getScaleFactor();
	else
		g_scale = 1.0;

	if (m_userSettings!=NULL)
		g_texScale = m_userSettings->getTextureScale();
	else
		g_texScale = 1.0;
	...
	switch (m_userSettings->getRepresentation()) {
	case FIBRE_BAND1:
		...
		m_userSettings->getCamera()->getUpVector(g_uv);
	...
	case FIBRE_BAND2:
		...
		m_userSettings->getCamera()->getForwardVector(g_ef[0],g_ef[1],g_ef[2]);
...
void CIvfFibre::saveToStream(ostream &out)
{
	m_coords->saveToStream(out);
}

void CIvfFibre::readFromStream(istream &in)
{
	m_coords->readFromStream(in);
}

void CIvfFibre::setUserSettings(CIvfUserSettings *settings)
{
...
void CIvfFibre::initSection()
{
	...
	if (m_userSettings!=NULL)
		r = m_userSettings->getFibreRadius();
	else
		r = 0.1;
	
	if (m_userSettings!=NULL)
		nSides = m_userSettings->getExtrusionSides();
	else
		nSides = 8;
