...
void GleContour::setSize(int size)
{
    ...
    m_normals = new gleDouble[m_size][2];
}
...
void ivf::GleContour::setClosed(bool closed)
{
    m_closed = closed;
}

bool ivf::GleContour::closed()
{
    return m_closed;
}
...
void GleContour::calcNormals()
{
    ...
    if (m_closed)
    {
        m_normals[m_size - 1][0] = m_normals[0][0];
        m_normals[m_size - 1][1] = m_normals[0][1];
    }
}
